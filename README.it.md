...
- 💧 **Acqua** — fiumi e laghi principali, case study su siccità/restringimento, livello idrometrico live per la Lombardia (sensori ARPA), temperatura del mare regionale (6 zone costiere, la più vicina a dove stai guardando sulla mappa) con un **trend delle medie della stagione calda (maggio–settembre) che risale al 1982**, overlay satellitare NDWI per vedere l'acqua superficiale direttamente, un overlay radar Sentinel-1 SAR che vede sotto le nuvole (l'NDWI ottico no) — utile per la mappatura delle alluvioni durante un temporale — un **overlay della temperatura superficiale del Mediterraneo che risale al 1982** (Copernicus Marine, prodotto L4 riprocessato — trascina lo slider dell'anno e guardi il bacino scaldarsi), e 14 punti sparsi in tutto il Mediterraneo con temperatura/livello del mare live (Open-Meteo Marine) più una narrazione con fonte su cosa succede alla fauna marina in quel sotto-bacino (ondate di calore marine, tropicalizzazione, fioriture di meduse — fenomeni reali documentati, non dati live).
...
- 🧊 **Contorni dei ghiacciai** — perimetri GLIMS/NSIDC come overlay da attivare *sopra* un'immagine Sentinel-2 recente (è un source separato da quello satellitare proprio per poterli combinare), in due campagne: 2000–2003 e 2013–2016, più una **modalità confronto** che le disegna insieme virando la tonalità della più vecchia — due epoche nello stesso rosa erano indistinguibili, e alternarle sembrava non cambiasse nulla; il bordo colorato che spunta da sotto è ghiaccio che non c'è più. Nessuna area in km², deliberatamente: GLIMS aggrega delineazioni di gruppi di ricerca diversi, e per lo stesso ghiacciaio nello stesso anno possono discordare di un ordine di grandezza (l'Adamello nel 2011 compare come 0,25 / 9,09 / 15,76 km²; confrontando due campagne coerenti per autore, 2003→2016, "crescerebbe" da 10,10 a 14,36 km²). Un grafico costruito su questo mostrerebbe ghiacciai che si espandono. I contorni invece sono geometrie reali, quindi il ritiro si mostra invece di calcolarlo — per i numeri le fonti sono WGMS (misure a terra, 448 ghiacciai italiani) e il Catasto dei Ghiacciai Italiani.
...
- **Dati meteo:** Open-Meteo (nessuna API key, CORS abilitato), inclusa la Marine API per la temperatura del mare attuale sui punti mediterranei
- **Storico temperatura del mare:** precalcolato in `src/data/sst-history.json` da `npm run sst:refresh` (`scripts/generate-sst-history.ts`), che campiona un giorno ogni ~10 da maggio a settembre di ogni anno dal 1982 (la stagione calda del mare, non l'estate del calendario: maggio mostra quando parte il riscaldamento, settembre è quando il calore accumulato torna all'atmosfera e alimenta i nubifragi di inizio autunno — misurato, il trend giugno-agosto corre a ~+0,5°C/decennio contro ~+0,27 di settembre-ottobre, quindi allargare all'anno intero diluirebbe invece di chiarire). Le stagioni campionate sotto l'80% vengono scartate invece che pubblicate: all'anno in corso mancano proprio i mesi più caldi, e la sua media sembrerebbe un raffreddamento improvviso mai avvenuto, per ciascuna delle 6 zone di mare, via `GetFeatureInfo` di Copernicus Marine. Precalcolato invece che chiesto live perché una media stagionale onesta richiede decine di giorni per anno e il servizio risponde un punto per richiesta — migliaia di richieste che nessun visitatore deve aspettare. Una GitHub Action (`refresh-sst-history.yml`) lo rigenera ogni ottobre, quando il prodotto riprocessato ha pubblicato l'estate appena passata, e fa il deploy solo se il file è davvero cambiato
- **Layer temperatura del mare:** WMTS pubblico di Copernicus Marine (CMEMS), dataset `SST_MED_SST_L4_REP_OBSERVATIONS_010_021` — giornaliero dal 1982-01-01, chiamato direttamente dal browser (verificato: nessuna credenziale richiesta, tile restituiti anche per il 1985 e il 2000). È il prodotto *riprocessato*, quindi resta indietro di circa un mese rispetto a oggi: le date più recenti vengono arretrate invece di restituire tile vuoti. La scala colore e il suo intervallo 17,8→28,5°C arrivano dal `GetLegend` del layer stesso, non inventati in locale
...
`public/robots.txt`, `public/sitemap.xml` e `public/og-image.jpg` (generata dal layer della temperatura del mare, 2400×1260). Prima, `robots.txt` e `sitemap.xml` *sembravano* esserci perché il fallback SPA di Nginx serviva `index.html` con un 200 per qualunque percorso sconosciuto — vale la pena saperlo, perché un controllo veloce li avrebbe dati per fatti. La sitemap dichiara una sola URL e spiega in un commento il perché: l'app tiene lo stato (modulo, layer, anno) in React e non nell'indirizzo, quindi non esistono `/mare` o `/ghiacciai` da dichiarare — né un modo per condividere "il Mediterraneo nel 1985" come link. È quella la modifica che renderebbe l'indicizzazione davvero utile.
